	...
	public class UserRepository : IUserRepository
	{
		...
		private async Task<string> GetAccessToken(ApplicationUser user, string jwtTokenId)
		{
			...
			var tokenDescriptor = new SecurityTokenDescriptor
			{
				...
				Issuer = "https://bussyvilla-api.com",
				Audience = "https://test-bussy-api.com",
		...
		public async Task<TokenDTO> RefreshAccessToken(TokenDTO tokentDTO)
		{
			// Find the existing refresh token
			var existingRefreshToken = await _db.RefreshTokens.FirstOrDefaultAsync(u => u.Refresh_Token == tokentDTO.RefreshToken);
			if (existingRefreshToken == null)
			{
				return new TokenDTO();
			}

			// Comapare data from existing refresh and access token provided and if there is any mismatch then consider it as a fraud
			var isTokenValid = GetAccessTokenData(tokentDTO.AccessToken, existingRefreshToken.UserId, existingRefreshToken.JwtTokenId);
			if (!isTokenValid)
			{
		...
		public async Task RevokeRefreshToken(TokenDTO tokentDTO)
		{
			var existingRefreshToken = await _db.RefreshTokens.FirstOrDefaultAsync(_ => _.Refresh_Token == tokentDTO.RefreshToken);
			if (existingRefreshToken == null)
				return;
